        ...
        public static Stream FileSignCadesBes(Stream xmlDocument, MyCert seleCert, bool isFirst = true, bool isParallelSignature = false)
        {
            ...
            try
            {
                if (!isParallelSignature)
                {
                    if (isFirst)
                    {
                        signedDocument = APB.CryptoLib.GSS.GssSign.SignXml(xmlDocument, certificate, DigestMethod.SHA1, cSignatureFormat, isParallelSignature, null);
                    }
                    else
                    {
                        signedDocument = APB.CryptoLib.GSS.GssSign.CoSignXml(xmlDocument, certificate, DigestMethod.SHA1, cSignatureFormat, null);
                    }
                }
                else
                {
