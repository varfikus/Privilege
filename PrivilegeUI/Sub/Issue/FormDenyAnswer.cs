...
{
    public partial class FormDenyAnswer : Form
    {
        #region Fields
        ...
        public FormDenyAnswer(MyHttpClient apiClient, Application app, FormMain parentForm)
        {
            InitializeComponent();
        ...
        private async void FormDenyAnswer_Load(object sender, EventArgs e)
        {
            try
            {
                if (_app.Id <= 0)
        ...
        private async Task<bool> CreateDoc()
        {
            ...
            if (!await GenerateHtmlxAndUploadAsync(_app.Idgosuslug, "Предоставление льготы для АСОП", tB_fio.Text, tB_operator.Text, tB_operatorTel.Text, tB_denial.Text))
                return false;
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<bool> GenerateHtmlxAndUploadAsync(string idGosUslugi, string service, string fio, string operatorName, string operatorPhone, string reason)
        {
            try
            {
                ...
                if (content != null)
                {
                    ...
                    content.Add(new XElement(ns + "p", styleAttr, new XText("В соответствии с Вашим ответом по заявке.")));
                    content.Add(new XElement(ns + "p", styleAttr, new XText("Ваш ответ отклонен по причине: \"" + reason + "\".")));
                }
                ...
                string ftpFilePath = $"DeniedApplications/Answer/{tempFileName}";
        ...
        private async Task<bool> UpdateAsync()
        {
            try
            {
                DeniedApplicationDto applicationDto = new DeniedApplicationDto
                {
                    Id = _app.Id,
                    Status = DenyStatus.Delivered
                };

                var result = await _apiClient.PutAsync<DeniedApplicationDto, BaseResult<DeniedApplicationDto>>("api/deniedapplications", applicationDto);
