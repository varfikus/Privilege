...
namespace Privilege.UI.Window.Client
{
    public partial class FormMain : Form
    {
        ...
        private void LoadTable()
        {
            _countAdd = 0;
            _countApply = 0;
            string office = UserInfo.RuleOffice == "0" ? "" : " AND office in (" + UserInfo.RuleOffice + ")";
            string service = UserInfo.RuleService == "0" ? "" : " AND service_id in (" + UserInfo.RuleService + ")";
            ...
            if (_isArchive)
            {
                query = "SELECT * FROM documents WHERE (status = '2' OR status = '4' OR status = '6')" +
                        office + service + condition +
                        "AND date_ispoln < '" + date.Year + "-" + date.Month.ToString("00") + "-" +
                        date.Day.ToString("00") + "%' " +
                        "ORDER BY date_add DESC";
            }
            else
            {
                string dateIspoln = " AND date_ispoln > '" + date.Year + "-" + date.Month.ToString("00") + "-" +
                                    date.Day.ToString("00") + "%' ORDER BY date_add DESC";

                query = "SELECT * FROM documents WHERE (status = 0 OR status = 1)" + office + service + condition +
                        "ORDER BY date_add DESC; " +
                        "SELECT * FROM documents WHERE (status = 3 OR status = 9)" + office + service + condition + "; " +
                        "SELECT * FROM documents WHERE (status = 5 OR status = 7 OR status = 8)" +
                        office + service + condition + ";" +
                        "SELECT * FROM documents WHERE (status = 2 OR status = 4 OR status = 6)" +
                        office + service + condition + dateIspoln;
            }
            ...
            try
            {
                ...
                if (!_isArchive)
                {
                    _countAdd = ds.Tables[0].Rows.Count;
                    _countApply = ds.Tables[1].Rows.Count;
                }
                FillTable(ds);
            }
            catch (Exception ex)
        ...
        private void FillTable(DataSet ds)
        {
            ...
            for (int i = 0; i < ds.Tables.Count; i++)
            {
                ...
                for (int j = 0; j < dt.Rows.Count; j++)
                {
                    ...
                    DateTime.TryParse(dt.Rows[j]["date_add"].ToString(), out DateTime dateAdd);

                    dr["npp"] = ++npp;
                    dr["id"] = dt.Rows[j]["id"].ToString();
                    dr["num_id"] = dateAdd.Year + "." + dt.Rows[j]["id"];
                    dr["fio"] = dt.Rows[j]["fio"].ToString();
                    dr["birthday"] = dt.Rows[j]["birthday"].ToString();
                    dr["address_reg"] = dt.Rows[j]["address_reg"].ToString();
                    dr["date_add"] = dateAdd.ToString("d");
                    switch (dt.Rows[j]["status"].ToString())
                    {
                        case "0":
                            dr["status"] = "Не принят";
                            break;
                    ...
                    TimeSpan subDate = Convert.ToDateTime(dt.Rows[j]["date_ispoln"]).Subtract(Convert.ToDateTime(DateTime.Now));
                    if (subDate.Days > 0 && (dt.Rows[j]["status"].ToString().ToLower() == "0" ||
                                             dt.Rows[j]["status"].ToString().ToLower() == "3" ||
                                             dt.Rows[j]["status"].ToString().ToLower() == "1"))
                        dr["days_left"] = subDate.Days;
                    else
                        dr["days_left"] = "0";
                    dr["date_ispoln"] = dt.Rows[j]["date_ispoln"].ToString();
                    dr["id_gosuslug"] = dt.Rows[j]["id_uslugi"].ToString();
                    dr["name_usl"] = dt.Rows[j]["name_usl"].ToString();
                    dr["region"] = dt.Rows[j]["region"].ToString();
                    if (!string.IsNullOrEmpty(dt.Rows[j]["office"].ToString()) && dt.Rows[j]["office"].ToString() != "0")
                        dr["office"] = _officeDictionary.Count != 0
                            ? _officeDictionary[Convert.ToInt32(dt.Rows[j]["office"].ToString())]
                            : dt.Rows[j]["office"].ToString();
                    dr["electronic"] = dt.Rows[j]["electronic"].ToString() == "True";
                    dr["service_id"] = dt.Rows[j]["service_id"].ToString();
